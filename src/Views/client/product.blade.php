            ...
            ?>
                <div class="col-lg-3 col-md-4 col-sm-6">
                    <div class="card border-0 rounded-0 text-center shadow-none overflow-hidden">
                        <a href="<?= url("client/{$item['id']}/details") ?>">
                            <span class="badge badge-primary">NEW</span>
                            <td><img src="{{ asset($item['img_thumbnail']) }}" alt="" width="300px"></td>
                            <div class="card-body">
                                <h4 class="text-uppercase mb-3"><?= $item['name'] ?></h4>
                                <p class="h4 text-muted font-weight-light mb-3">Lip Gloss</p>
                                <p class="h4">$<?php echo $item['overview']; ?></p>
                            </div>
                        </a>
                    </div>
                </div>
                ...
                <div class="col-12 text-center mt-5">
                        ...
                        <ul class="pagination">
                            <li class="page-item {{ $page == 1 ? 'disabled' : '' }}">
                                <a class="page-link " href=" {{ url('client/product?page=' . $page - 1) }}"
                                    aria-label="Previous">
                            ...
                            @for ($i = 1; $i <= $totalPage; $i++)
                                <li class="page-item {{ $i == $page ? 'active' : '' }}"><a class="page-link"
                                        href="{{ url('client/product?page=') . $i }}">{{ $i }}</a></li>
                            @endfor

                            <li class="page-item {{ $page == $totalPage ? 'disabled' : '' }}">
                                <a class="page-link "
                                    href="{{ $page < $totalPage ? url('client/product?page=' . $page + 1) : '' }}"
                                    aria-label="Next">
